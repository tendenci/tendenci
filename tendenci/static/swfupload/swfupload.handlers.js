...
function fileDialogComplete(num_files_queued) {
	...
	if(!files_queued) return; // if no files; do nothing

	// remove initial interface
	$('#swf-progress-init').remove();
	$('#swf-msg-mask').remove();
	$('#SWFUpload_0').height('0');	// removing or hiding this element (the flash movie)
									// breaks the swfuploader; (the movie doubles as a button)
	this.startUpload();
}
