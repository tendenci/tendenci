...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Nav.objects.exclude(megamenu=True).update(megamenu=False)
        # Fields from TendenciBaseModel
        orm.Nav.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.Nav.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.Nav.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.Nav.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.Nav.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.Nav.objects.exclude(status=True).update(status=False)
        # End of Fields from TendenciBaseModel

        orm.NavItem.objects.exclude(new_window=True).update(new_window=False)
