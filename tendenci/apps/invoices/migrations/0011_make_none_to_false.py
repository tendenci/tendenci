...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Invoice.objects.exclude(is_void=True).update(is_void=False)
        orm.Invoice.objects.exclude(status=True).update(status=False)
        orm.Invoice.objects.exclude(receipt=True).update(receipt=False)
        orm.Invoice.objects.exclude(gift=True).update(gift=False)
        orm.Invoice.objects.exclude(tax_exempt=True).update(tax_exempt=False)
        orm.Invoice.objects.exclude(taxable=True).update(taxable=False)
