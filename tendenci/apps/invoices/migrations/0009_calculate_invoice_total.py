...
class Migration(DataMigration):
    ...
    depends_on = (
        ('profiles', '0014_auto__add_field_profile_total_spend'),
    )

    def forwards(self, orm):
        """
        Calculates total invoice hours and put it in total_spend
        """
        for user in orm['auth.User'].objects.all():
            ...
            if not profile:
                profile, created = orm['profiles.Profile'].objects.get_or_create(
                    user=user,
                    creator_id=user.id,
            ...
            # filters from invoices/views.py@top_spenders
            filters = (Q(creator=user) | Q(owner=user) | Q(bill_to_email=user.email)) & Q(status_detail='tendered')

            # get all invoices
            invoices = orm['invoices.Invoice'].objects.filter(filters) \
                .aggregate(Sum('total'))

            # record total_spend
            profile.total_spend = invoices['total__sum'] or 0
            profile.save()
    ...
    complete_apps = ['profiles', 'invoices']
    symmetrical = True
