...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.PhotoSize.objects.exclude(upscale=True).update(upscale=False)
        orm.PhotoSize.objects.exclude(crop=True).update(crop=False)
        orm.PhotoSize.objects.exclude(pre_cache=True).update(pre_cache=False)
        orm.PhotoSize.objects.exclude(increment_count=True).update(increment_count=False)

        orm.Image.objects.exclude(is_public=True).update(is_public=False)
        orm.Image.objects.exclude(is_public=True).update(is_public=False)
        # Fields from TendenciBaseModel
        orm.Image.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.Image.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.Image.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.Image.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.Image.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.Image.objects.exclude(status=True).update(status=False)
        # End of Fields from TendenciBaseModel

        # Fields from TendenciBaseModel
        orm.PhotoSet.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.PhotoSet.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.PhotoSet.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.PhotoSet.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.PhotoSet.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.PhotoSet.objects.exclude(status=True).update(status=False)
        # End of Fields from TendenciBaseModel
