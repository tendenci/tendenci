...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Job.objects.exclude(is_agency=True).update(is_agency=False)
        orm.Job.objects.exclude(syndicate=True).update(syndicate=False)
        orm.Job.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.Job.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.Job.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.Job.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.Job.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.Job.objects.exclude(status=True).update(status=False)

        orm.JobPricing.objects.exclude(show_member_pricing=True).update(show_member_pricing=False)
        orm.JobPricing.objects.exclude(include_tax=True).update(include_tax=False)
        orm.JobPricing.objects.exclude(status=True).update(status=False)
