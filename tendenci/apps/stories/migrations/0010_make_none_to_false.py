...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Story.objects.exclude(syndicate=True).update(syndicate=False)
        orm.Story.objects.exclude(expires=True).update(expires=False)
        # Fields from TendenciBaseModel
        orm.Story.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.Story.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.Story.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.Story.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.Story.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.Story.objects.exclude(status=True).update(status=False)
        # End of Fields from TendenciBaseModel
    ...
    models = {
        ...
        u'stories.story': {
            ...
            'expires': ('django.db.models.fields.NullBooleanField', [], {'default': 'True', 'null': 'True', 'blank': 'True'}),
