...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Discount.objects.exclude(never_expires=True).update(never_expires=False)
        # Fields from TendenciBaseModel
        orm.Discount.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.Discount.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.Discount.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.Discount.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.Discount.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.Discount.objects.exclude(status=True).update(status=False)
    ...
    models = {
        ...
        u'discounts.discount': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'never_expires': ('django.db.models.fields.NullBooleanField', [], {'null': 'True', 'blank': 'True'}),
