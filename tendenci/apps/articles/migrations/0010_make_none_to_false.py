...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Article.objects.exclude(syndicate=True).update(syndicate=False)
        orm.Article.objects.exclude(featured=True).update(featured=False)
        orm.Article.objects.exclude(not_official_content=True).update(not_official_content=False)

        # Fields from TendenciBaseModel
        orm.Article.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.Article.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.Article.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.Article.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.Article.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.Article.objects.exclude(status=True).update(status=False)
