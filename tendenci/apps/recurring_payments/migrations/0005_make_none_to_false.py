...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.RecurringPayment.objects.exclude(tax_exempt=True).update(tax_exempt=False)
        orm.RecurringPayment.objects.exclude(taxable=True).update(taxable=False)
        orm.RecurringPayment.objects.exclude(has_trial_period=True).update(has_trial_period=False)
        orm.RecurringPayment.objects.exclude(status=True).update(status=False)

        orm.PaymentProfile.objects.exclude(status=True).update(status=False)

        orm.PaymentTransaction.objects.exclude(status=True).update(status=False)
