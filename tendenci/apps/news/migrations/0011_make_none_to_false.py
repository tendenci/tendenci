...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.News.objects.exclude(syndicate=True).update(syndicate=False)
        orm.News.objects.exclude(use_auto_timestamp=True).update(use_auto_timestamp=False)
        # Fields from TendenciBaseModel
        orm.News.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.News.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.News.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.News.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.News.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.News.objects.exclude(status=True).update(status=False)
