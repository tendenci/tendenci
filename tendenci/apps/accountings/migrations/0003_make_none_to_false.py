...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.AcctEntry.objects.exclude(exported=True).update(exported=False)
        orm.AcctEntry.objects.exclude(status=True).update(status=False)
        orm.AcctTran.objects.exclude(cleared=True).update(cleared=False)
        orm.AcctTran.objects.exclude(status=True).update(status=False)
