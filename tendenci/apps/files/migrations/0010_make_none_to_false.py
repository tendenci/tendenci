...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.File.objects.exclude(is_public=True).update(is_public=False)

        orm.File.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.File.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.File.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.File.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.File.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.File.objects.exclude(status=True).update(status=False)
