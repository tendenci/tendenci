...
# The post_migrate signal is not being called because
...
# command in order to continue using the post_migrate signal
...
if "tendenci.apps.notifications" in settings.INSTALLED_APPS:
    ...
    def create_notice_types(app, verbosity, **kwargs):
        notification.create_notice_type("user_added", _("User Added"), _("A user has been added."), verbosity=verbosity)
        notification.create_notice_type("user_edited", _("User Edited"), _("A user has been edited."), verbosity=verbosity)
        notification.create_notice_type("user_deleted", _("User Deleted"), _("A user has been deleted"), verbosity=verbosity)
