...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Payment.objects.exclude(payment_attempted=True).update(payment_attempted=False)
        orm.Payment.objects.exclude(verified=True).update(verified=False)
        orm.Payment.objects.exclude(status=True).update(status=False)

        orm.PaymentMethod.objects.exclude(is_online=True).update(is_online=False)
        orm.PaymentMethod.objects.exclude(admin_only=True).update(admin_only=False)
