...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Group.objects.exclude(show_as_option=True).update(show_as_option=False)
        orm.Group.objects.exclude(allow_self_add=True).update(allow_self_add=False)
        orm.Group.objects.exclude(allow_self_remove=True).update(allow_self_remove=False)
        orm.Group.objects.exclude(sync_newsletters=True).update(sync_newsletters=False)
        orm.Group.objects.exclude(auto_respond=True).update(auto_respond=False)
        # Fields from TendenciBaseModel
        orm.Group.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.Group.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.Group.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.Group.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.Group.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.Group.objects.exclude(status=True).update(status=False)
        # End of Fields from TendenciBaseModel

        orm.GroupMembership.objects.exclude(status=True).update(status=False)
