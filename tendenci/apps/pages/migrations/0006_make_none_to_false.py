...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Page.objects.exclude(syndicate=True).update(syndicate=False)
        orm.Page.objects.exclude(view_contact_form=True).update(view_contact_form=False)
        # Fields from TendenciBaseModel
        orm.Page.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.Page.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.Page.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.Page.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.Page.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.Page.objects.exclude(status=True).update(status=False)
