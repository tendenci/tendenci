...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.MembershipType.objects.exclude(require_approval=True).update(require_approval=False)
        orm.MembershipType.objects.exclude(require_payment_approval=True).update(require_payment_approval=False)
        orm.MembershipType.objects.exclude(allow_renewal=True).update(allow_renewal=False)
        orm.MembershipType.objects.exclude(renewal=True).update(renewal=False)
        orm.MembershipType.objects.exclude(renewal_require_approval=True).update(renewal_require_approval=False)
        orm.MembershipType.objects.exclude(admin_only=True).update(admin_only=False)
        orm.MembershipType.objects.exclude(never_expires=True).update(never_expires=False)
        orm.MembershipType.objects.exclude(fixed_option2_can_rollover=True).update(fixed_option2_can_rollover=False)
        # Fields from TendenciBaseModel
        orm.MembershipType.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.MembershipType.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.MembershipType.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.MembershipType.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.MembershipType.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.MembershipType.objects.exclude(status=True).update(status=False)
        # End of Fields from TendenciBaseModel

        orm.MembershipDefault.objects.exclude(renewal=True).update(renewal=False)
        orm.MembershipDefault.objects.exclude(application_abandoned=True).update(application_abandoned=False)
        orm.MembershipDefault.objects.exclude(application_complete=True).update(application_complete=False)
        orm.MembershipDefault.objects.exclude(application_approved=True).update(application_approved=False)
        orm.MembershipDefault.objects.exclude(application_denied=True).update(application_denied=False)
        orm.MembershipDefault.objects.exclude(override=True).update(override=False)
        orm.MembershipDefault.objects.exclude(exported=True).update(exported=False)
        orm.MembershipDefault.objects.exclude(government_worker=True).update(government_worker=False)
        # Fields from TendenciBaseModel
        orm.MembershipDefault.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.MembershipDefault.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.MembershipDefault.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.MembershipDefault.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.MembershipDefault.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.MembershipDefault.objects.exclude(status=True).update(status=False)
        # End of Fields from TendenciBaseModel

        orm.Notice.objects.exclude(system_generated=True).update(system_generated=False)
        orm.Notice.objects.exclude(status=True).update(status=False)

        orm.NoticeDefaultLogRecord.objects.exclude(action_taken=True).update(action_taken=False)

        orm.MembershipApp.objects.exclude(use_captcha=True).update(use_captcha=False)
        orm.MembershipApp.objects.exclude(allow_multiple_membership=True).update(allow_multiple_membership=False)
        orm.MembershipApp.objects.exclude(include_tax=True).update(include_tax=False)
        orm.MembershipApp.objects.exclude(discount_eligible=True).update(discount_eligible=False)
        orm.MembershipApp.objects.exclude(use_for_corp=True).update(use_for_corp=False)
        # Fields from TendenciBaseModel
        orm.MembershipApp.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.MembershipApp.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.MembershipApp.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.MembershipApp.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.MembershipApp.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.MembershipApp.objects.exclude(status=True).update(status=False)
        # End of Fields from TendenciBaseModel


        orm.MembershipAppField.objects.exclude(required=True).update(required=False)
        orm.MembershipAppField.objects.exclude(display=True).update(display=False)
        orm.MembershipAppField.objects.exclude(admin_only=True).update(admin_only=False)
