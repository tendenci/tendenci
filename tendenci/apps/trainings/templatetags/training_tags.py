...
def show_certification(cert, customer):
...
@register.simple_tag
def get_diamond_image(num_diamonds):
    num_diamonds = int(num_diamonds)      
    file = get_setting('module', 'trainings', f"diamondimage{num_diamonds}")
    if not file:
        return None
    return reverse('file', args=[file])
