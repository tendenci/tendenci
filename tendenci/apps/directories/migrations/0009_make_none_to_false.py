...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Directory.objects.exclude(renewal_notice_sent=True).update(renewal_notice_sent=False)
        orm.Directory.objects.exclude(syndicate=True).update(syndicate=False)
        # Fields from TendenciBaseModel
        orm.Directory.objects.exclude(allow_anonymous_view=True).update(allow_anonymous_view=False)
        orm.Directory.objects.exclude(allow_user_view=True).update(allow_user_view=False)
        orm.Directory.objects.exclude(allow_member_view=True).update(allow_member_view=False)
        orm.Directory.objects.exclude(allow_user_edit=True).update(allow_user_edit=False)
        orm.Directory.objects.exclude(allow_member_edit=True).update(allow_member_edit=False)
        orm.Directory.objects.exclude(status=True).update(status=False)

        orm.DirectoryPricing.objects.exclude(show_member_pricing=True).update(show_member_pricing=False)
        orm.DirectoryPricing.objects.exclude(status=True).update(status=False)
