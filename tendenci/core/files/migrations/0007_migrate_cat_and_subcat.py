...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for file_item in orm.File.objects.all():
            try:
                f = File.objects.get(pk=file_item.id)
                cat_set = f.category_set
                cat = cat_set.get('category', None)
                sub_cat = cat_set.get('sub_category', None)
                category = None
                sub_category = None

                if cat:
                    category = orm.FilesCategory.objects.filter(name=cat.name)
                    if category.exists():
                        category = category[0]
                    else:
                        category = orm.FilesCategory.objects.create(name=cat.name)

                    file_item.file_cat = category

                    if sub_cat:
                        sub_category = orm.FilesCategory.objects.filter(name=sub_cat.name)
                        if sub_category.exists():
                            sub_category = sub_category[0]
                        else:
                            sub_category = orm.FilesCategory.objects.create(name=sub_cat.name, parent=category)

                        file_item.file_sub_cat = sub_category
