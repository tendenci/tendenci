...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        #transfer photologue's photosizes to photo's photosizes.
        for size in orm['photologue.PhotoSize'].objects.all():
            new_size = orm.PhotoSize()
            new_size.name = size.name
            new_size.width = size.width
            new_size.height = size.height
            new_size.quality = size.quality
            new_size.upscale = size.upscale
            new_size.crop = size.crop
            new_size.pre_cache = size.pre_cache
            new_size.increment_count = size.increment_count
            if size.effect:
                try:
                    new_effect = orm.PhotoEffect.objects.get(pk=size.effect.pk)
                except orm.PhotoEffect.DoesNotExist:
                    new_effect = orm.PhotoEffect()
                    new_effect.name = size.effect.name
                    new_effect.description = size.effect.description
                    new_effect.transpose_method = size.effect.transpose_method
                    new_effect.color = size.effect.color
                    new_effect.brightness = size.effect.brightness
                    new_effect.contrast = size.effect.contrast
                    new_effect.sharpness = size.effect.sharpness
                    new_effect.filters = size.effect.filters
                    new_effect.reflection_size = size.effect.reflection_size
                    new_effect.reflection_strength = size.effect.reflection_strength
                    new_effect.background_color = size.effect.background_color
                    #match the pks so we know if we've already transfered the data
                    new_effect.pk = size.effect.pk
                    new_effect.save()
                new_size.effect = new_effect
            
            if size.watermark:
                try:
                    new_mark = orm.Watermark.objects.get(pk=size.watermark.pk)
                except orm.Watermark.DoesNotExist:
                    new_mark = orm.Watermark()
                    new_mark.name = size.watermark.name
                    new_mark.description = size.watermark.description
                    new_mark.image = size.watermark.image
                    new_mark.style = size.watermark.style
                    new_mark.opacity = size.watermark.opacity
                    #match the pk's so we know if we've already transfered the data
                    new_mark.pk = size.watermark.pk
                    new_mark.save()
                new_size.watermark = new_mark
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 30, 10, 54, 18, 229434)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 30, 10, 54, 18, 229096)'}),
    ...
    complete_apps = ['photologue', 'photos']
