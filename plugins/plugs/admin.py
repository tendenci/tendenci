...
from plugs.forms import PlugForm
...
class PlugAdmin(admin.ModelAdmin):
    ...
    def edit_link(self, obj):
        link = '<a href="%s" title="edit">Edit</a>' % reverse('admin:plugs_plug_change', args=[obj.pk])
        return link
    ...
    def view_on_site(self, obj):
        ...
        link = '<a href="%s" title="%s"><img src="%s" /></a>' % (
            reverse('plugs.detail', args=[obj.pk]),
            obj,
            link_icon,
        )
        return link
    ...
    def log_deletion(self, request, object, object_repr):
        super(PlugAdmin, self).log_deletion(request, object, object_repr)
        log_defaults = {
            'event_id' : 100300,
    ...
    def log_change(self, request, object, message):
        super(PlugAdmin, self).log_change(request, object, message)
        log_defaults = {
            'event_id' : 100200,
    ...
    def log_addition(self, request, object):
        super(PlugAdmin, self).log_addition(request, object)
        log_defaults = {
            'event_id' : 100100,
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        """
        form = super(PlugAdmin, self).get_form(request, obj, **kwargs)
    ...
    def change_view(self, request, object_id, extra_context=None):
        result = super(PlugAdmin, self).change_view(request, object_id, extra_context)
