...
function update_form_fields(form,original_form,form_number,remove){form_number=parseInt(form_number);var search='-'+(form_number)+'-';var replacement='-'+(form_number+1)+'-';var rep_label='$1&nbsp;#'+(form_number+2);var rep_form_number=form_number+1;if(remove){replacement='-'+(form_number-1)+'-';rep_label='$1&nbsp;#'+(form_number);rep_form_number=form_number-1;}
form.find('div[class]').each(function(){var _class=$(this).attr('class').replace(search,replacement);$(this).attr('class',_class);});form.find(':input').each(function(){var name=$(this).attr('name').replace(search,replacement);var id='id_'+name;$(this).attr({'name':name,'id':id}).val('').removeAttr('checked');});form.find('label').each(function(){var newFor=$(this).attr('for').replace(search,replacement);$(this).attr('for',newFor);});if(original_form){original_form.find('input[type="text"]').each(function(){var input_value=$(this).val();var name=$(this).attr('name').replace(search,replacement);form.find('input[name='+name+']').val(input_value);});}
form.find('input[name="form-number"]').val(rep_form_number);return form;}
function clone_form(selector,type){var current_element=$(selector);var new_element=current_element.clone(true);var form_functions=current_element.next();var form_functions_clone=form_functions.clone(true);var total=parseInt($('#'+type+'-TOTAL_FORMS').val());var form_number=current_element.find('input[name="form-number"]').val();new_element=update_form_fields(new_element,current_element,form_number,false);total++;$('#'+type+'-TOTAL_FORMS').val(total);new_element.hide();current_element.after(new_element);new_element.fadeIn(200);new_element.before(form_functions_clone);if(total>1){var delete_link=form_functions.find('div.formset-delete');delete_link.css({display:'inline',marginLeft:'15px'});var prev_add_link=form_functions_clone.find('div.formset-add');prev_add_link.hide();var prev_delete_link=form_functions_clone.find('div.formset-delete');prev_delete_link.css({display:'inline',marginLeft:'0px'});}}
function delete_form(current_form,selector,type){var current_form_number=current_form.find('input[name="form-number"]').val();var forms_prev=current_form.prevAll(selector+':visible');var forms_after=current_form.nextAll(selector+':visible');var prev_form_functions=current_form.prev('div.formset-functions:visible');var prev_form=prev_form_functions.prev(':visible');var total=$('#'+type+'-TOTAL_FORMS').val();var href=window.location.href;var is_edit_page=(href.indexOf('events/edit')>-1);var form_functions=prev_form_functions;if(form_functions.length==0){if(forms_after.length==1){form_functions=$(forms_after[0]).next();}else{form_functions=current_form.next();}}
if(!is_edit_page){total--;$('#'+type+'-TOTAL_FORMS').val(total);}
if(!is_edit_page){if(forms_after.length>=1){forms_after.each(function(){var form=$(this);var form_number=form.find('input[name="form-number"]').val();update_form_fields(form,null,form_number,true);});}};if((forms_prev.length==0)&&(forms_after.length==0)){var delete_link=form_functions.find('div.formset-delete');delete_link.hide();}
