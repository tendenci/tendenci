...
@permission_required('image.add_photo')
...
def file_upload(request):
    ...
    # get image filename via HEADERS
    # cannot use POST because i'm collecting binary data through it
    name = request.META.get('HTTP_FILENAME', 'untitled.jpg')
    set_id = request.META.get('HTTP_SET_ID', '0')

    # clean set_id
    try: set_id = int(set_id)
    except: set_id = 0

    file = SimpleUploadedFile(name, request.raw_post_data, 'image/jpeg')

    q = QueryDict('')
    post = q.copy() # to make it mutable
    files = q.copy() # to make it mutable

    # post update
    post.update({
        'image': file.name, # filename
        'title': file.name[::-1].split(".", 1)[1][::-1], # filename sans-extension
        'safetylevel': 1
    })

    # files update
    files.update({'image': file})

    photo_form = PhotoUploadForm(request.user, post, files)
    ...
    if photo_form.is_valid():
        ...
        return HttpResponse("photo is not valid", mimetype="image/jpeg")
...
#    photos = SearchQuerySet().models(Image)

    photos = Image.objects.all()
    
#    if q: photos = photos.filter(content=q)
#    else: photos = photos.order_by("-date_added")

    # not authenticated; limit view
#    if not request.user.has_perm('photos.view_image'):
#        photos = photos.filter(is_public=True) # public photos
        
    return render_to_response(template_name, {"photos": photos, "q": q }, 
        context_instance=RequestContext(request))
