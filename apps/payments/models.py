...
class Payment(models.Model):
    ...
    address2 = models.CharField(max_length=60, default='', null=True)
    city = models.CharField(max_length=40, default='', null=True)
    state = models.CharField(max_length=40, default='', null=True)
    zip = models.CharField(max_length=20, default='', null=True)
    country = models.CharField(max_length=60, default='', null=True)
    phone = models.CharField(max_length=25, default='', null=True)
    fax = models.CharField(max_length=25, default='', null=True)
    ...
    def payments_pop_by_invoice_user(self, user, inv, session_id='', **kwargs):
        ...
        if inv.allow_payment_by(user, session_id):
            ...
            # default description
            self.description = 'Tendenci Invoice %d Payment (%d).' % (inv.id, inv.object_id)
            ...
            if obj:
                if hasattr(obj, 'get_payment_description'):
                    ...
                    if description:
                        ...
                        self.description = 'Tendenci Invoice %d for %s(%d).' % (inv.id, obj, inv.object_id)
                else:
                    self.description = 'Tendenci Invoice %d for %s(%d).' % (inv.id, obj, inv.object_id)
