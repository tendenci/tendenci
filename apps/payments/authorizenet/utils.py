...
def authorizenet_thankyou_processing(response_d, **kwargs):
    ...
    if payment.invoice.balance > 0:     # if balance=0, it means already processed
        payment_update_authorizenet(response_d, payment)
        payment_processing_object_updates(payment)
    return payment
        
def payment_update_authorizenet(response_d, payment, **kwargs):
    ...
    if payment.is_approved:
        ...
        payment.save()
        payment.invoice.make_payment(payment.amount)
    else:
